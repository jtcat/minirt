...
/*   Updated: 2024/03/31 17:35:02 by jcat             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cam_init(t_camera *cam)
{
	...
	cam->viewport_height = cam->viewport_width / ((double)(cam->image_width) / cam->image_height);
	cam->w = v3unit(cam->lookat);
	...
	viewport_ul = v3sub(v3sub(cam->lookfrom, v3scalef(cam->w, cam->focal_length)), v3scalef(v3sub(viewport_u, viewport_v), 0.5));
	cam->pix_ul_p = v3sum(viewport_ul, v3scalef(v3sum(cam->pix_du, cam->pix_dv), 0.5));
}
